...
if __name__ == '__main__':
    ...
    save_vid = args.vid
    if save_vid:
        output = cv2.VideoWriter('output.avi',fourcc, 5.0, (2*w, h))
    ...
    while True:
        ...
        if background is not None:
            ...
            rs1 = np.hstack([frame, np.zeros(frame.shape, np.uint8)])
            rs2 = np.hstack([fg_matt, fg_seg])

            rs = np.vstack([rs1, rs2])
            rs = cv2.resize(rs, (int(3*w/2), int(3*h/2)))
